...
| `project_service_manifest::compute_current_project_service_manifest` | Yes. `src/project-service-manifest.ts` exported `computeCurrentProjectServiceManifest`, and `src/multiplexer/runtime-guard.ts` used the drift result during dashboard runtime guard probing. | Fixed in `14bdbdb9` and `8697f1b9`: Rust `probe_runtime_guard()` now calls `has_project_service_build_drift()` before endpoint/service checks, matching Node's self-drift priority. Residual note: this is not honestly drivable from the daily-loop live residual without mutating the real binary under test or changing the launcher environment being tested. |
| `project_service_manifest::has_project_service_build_drift` | Yes. `probeRuntimeGuard()` called `hasProjectServiceBuildDrift()` before endpoint and service checks. | Fixed in `14bdbdb9` and `8697f1b9`: the production runtime guard probe supplies `selfDrift` from the current-manifest drift check instead of leaving the helper as a fixture-only seam. |
...
| `terminal_key_parser::is_shifted_letter_command` | Yes. Node called `isShiftedLetterCommand` from dashboard control, dashboard interaction, and help/navigation paths. | Fixed in `f188e6f6`: dashboard production key handling routes shifted-letter commands through the parser helper. Live residual proof: `41fffb80` drives modified-Shift-L through the real PTY key path and mutation-proves `daily-loop-shift-library-missing`. |
...
| `dashboard_terminal::read_dashboard_key` | Yes for dashboard key reading. Node dashboard input handlers parsed terminal bytes into key events before dispatch. | Rust production calls `read_dashboard_keys` from the live dashboard loop, preserving coalesced and pasted input. The single-key wrapper is only a unit-test convenience. |
| `dashboard_tui_visibility::read_tmux_tui_visibility_from_values` | Yes for the visibility decision. Node production called `readTmuxTuiVisibility()` and kept the injected query/value path for tests. | Rust production calls `read_tmux_tui_visibility` from the dashboard loop. The `_from_values` entrypoint is the fixture parser that supplies captured tmux/process rows without shelling out. |
...
| `daemon::runtime::RealDaemonRuntime::with_project_service_launcher_and_process_verifier` | No Node helper equivalent. | Dependency-injection constructor for tests that need fake launch and process verification. Production uses `RealDaemonRuntime::new` or `with_project_service_launcher` with the system verifier. |
...
1. Hosted audit prompt/device persistence and hosted operator streaming: decide

# Aimux Full Reference

This page preserves the former long-form README as a detailed reference. For
install and quick start, use the root [README](../README.md).
...
For the lifecycle model, see [runtime-lifecycle.md](runtime-lifecycle.md).
For the current source of truth, see [current-architecture.md](current-architecture.md).
For the original Tauri/Svelte client contract (historical, superseded by `app/`), see [desktop-ui-contract.md](desktop-ui-contract.md).
For the migration rationale, see [global-control-plane-rfc.md](global-control-plane-rfc.md).
...
See [dev-runtime.md](dev-runtime.md) for the full local GUI/backend loop.
...
- the broader latency architecture plan is documented in [latency-architecture-rfc.md](latency-architecture-rfc.md)
- the current entrypoint split is documented in [latency-entrypoints.md](latency-entrypoints.md)
...
For future tool wiring and continuity expectations, see [tool-integration.md](tool-integration.md).
...
For deeper details, see [tool-integration.md](tool-integration.md).
...
```bash
yarn release:patch   # X.Y.Z -> X.Y.(Z+1)
yarn release:minor   # X.Y.Z -> X.(Y+1).0
yarn release:major   # X.Y.Z -> (X+1).0.0
```

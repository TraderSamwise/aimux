# Async Cutover — Execution Plan v2

This is the plan as built on `feat/async-cutover`. Version 1 remains as the
pre-work plan; this version records the migration shape that actually emerged,
the gates now protecting it, and the open failures that still block calling the
cutover stable.
...
behaviour. The important change from v1 is that the work stopped being a
surface-by-surface thread replacement and became a seam deletion effort: every
sync/async bridge has to be named, justified, and eventually removed unless it
is a true process-entry boundary.
...
stream conversions, hosted and relay conversions, and async leftovers for local
UI and route families.

`yarn verify:full` has passed end to end on the branch. At the first full green
after the local UI response fix, it reported:
...
That pass proves the release lane is now an actual gate for this branch. It does
not close the open runtime failures listed below.
...
- **Hosted `UND_ERR_SOCKET`.** Hosted proxy requests still fail on the
  `/agents/output` path while shorter hosted requests work. The local UI
  `ECONNRESET` fix points at the likely class: an async listener closing before
  request-head or response flushing obligations are satisfied. The guarantee may
  belong once in the shared daemon/listener path rather than being repeated per
  listener.
- **Expose-empty.** Expose still has a live empty-tile failure. Earlier work
  restored missing first-frame captures without recapturing seeded snapshots,
  but this open failure means the visual path still needs live investigation
  rather than a fixture-only update.

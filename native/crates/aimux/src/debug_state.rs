...
fn read_json_source(path: &str) -> Value {
...
fn read_yaml_source(path: &str) -> Value {
    if !Path::new(path).exists() {
        return json!({ "status": "missing", "path": path });
    }
    match fs::read_to_string(path) {
...
fn source_unavailable(reason: &str) -> Value {
    json!({ "status": "unavailable", "reason": reason })
}

fn source_without_value(source: Value) -> Value {
...
fn source_with_value(source: &Value, value: Value) -> Value {
...
fn source_status(source: &Value) -> Option<&str> {
    source.get("status").and_then(Value::as_str)
}

fn source_value(source: &Value) -> Option<&Value> {
    source.get("value")
}
...
fn matches_string(candidate: Option<&str>, target: &str) -> bool {
...
fn service_canonical(id: Option<&str>, worktree_path: Option<&str>) -> String {
    id.map(|id| format!("service:{id}"))
        .unwrap_or_else(|| format!("service:{}", worktree_path.unwrap_or("unknown")))
}

fn session_canonical(id: Option<&str>, backend_session_id: Option<&str>) -> String {
...
fn worktree_canonical(path: Option<&str>, name: Option<&str>) -> String {
    format!("worktree:{}", path.or(name).unwrap_or("unknown"))
}

fn add_match(matches: &mut Vec<Value>, seen: &mut BTreeSet<String>, mut item: Value) {

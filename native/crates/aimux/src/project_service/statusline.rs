...
fn render_control_session_segment(session: &Value, label: &str) -> String {
    ...
    let detail = trim_text(
        &[Some(compact_control_session_title(session)), hint]
            .into_iter()
            .flatten()
            .collect::<Vec<_>>()
            .join(" "),
...
fn compact_session_title(session: &Value) -> String {
    compact_session_title_with_role(session, project_control_display_role(Some(session)))
}

fn compact_control_session_title(session: &Value) -> String {
    let role = project_control_display_role(Some(session)).filter(|role| {
        !((*role == "overseer" && is_overseer_session(Some(session)))
            || (*role == "scribe" && is_scribe_session(Some(session))))
    });
    compact_session_title_with_role(session, role)
}

fn compact_session_title_with_role(session: &Value, role: Option<&str>) -> String {
    if string_field(session, "kind") == Some("service")
        && let Some(command) =
    ...
    if string_field(session, "kind") == Some("service") {
        let tool = string_field(session, "tool").unwrap_or("service");
    ...
    let tool = string_field(session, "tool")
        ...
        .unwrap_or_else(|| string_field(session, "id").unwrap_or("agent"));
    let base = string_field(session, "label")
        .filter(|label| !is_autogenerated_statusline_label(tool, label))
        .unwrap_or(tool);
    match role {
        Some(role) if !role.trim().is_empty() => format!("{base}({role})"),
        _ => base.to_owned(),
    }
}

fn is_autogenerated_statusline_label(tool: &str, label: &str) -> bool {

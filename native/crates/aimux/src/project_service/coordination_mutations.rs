...
pub fn route_coordination_mutation_request_with_runtime(
    ...
    Some(deliver_response(context, pathname, body, response, runtime))
}
...
fn deliver_response(
    ...
    let plan = match delivery_plan(pathname, request_body, &response.body) {
        Some(plan) => plan,
        None => return response,
    };
    let delivered_to = deliver_prompt_to_recipients(context, &plan, runtime);
    object_insert_mut(&mut response.body, "deliveredTo", json!(delivered_to));
    ...
    ) && !delivered_to.is_empty()
    {
        ...
        let delivered_message =
            mark_message_value_delivered(response.body["message"].clone(), &delivered_to);
...
fn delivery_plan(pathname: &str, request_body: &Value, response: &Value) -> Option<DeliveryPlan> {
    match pathname {
        routes::threads::SEND => thread_message_delivery_plan(
            response,
            string_array_field(request_body, "to"),
            route_recipients(request_body),
            &string_field_with_default(request_body, "from", "user"),
        ),
        routes::handoff::SEND => thread_message_delivery_plan(
            response,
            string_array_field(request_body, "to"),
            route_recipients(request_body),
            &string_field_with_default(request_body, "from", "user"),
        ),
        routes::handoff::ACCEPT | routes::handoff::COMPLETE => thread_message_delivery_plan(
            response,
            Vec::new(),
            Vec::new(),
            &string_field_with_default(request_body, "from", "user"),
        ...
        routes::reviews::REQUEST_CHANGES => {
            task_outcome_delivery_plan(response, "review changes requested")
        }
        _ => None,
    }
}
...
fn task_assignment_delivery_plan(response: &Value) -> Option<DeliveryPlan> {
    ...
    let recipient = trimmed_string(task.get("assignedTo"))?;
    Some(DeliveryPlan {
        recipients: vec![recipient.clone()],
        prompt: task_assignment_prompt(task, thread, message, RECIPIENT_PLACEHOLDER),
    })
}

fn task_outcome_delivery_plan(response: &Value, action: &str) -> Option<DeliveryPlan> {
    let task = response.get("task")?;
    let thread = response.get("thread").unwrap_or(&Value::Null);
    let message = response.get("message").unwrap_or(&Value::Null);
    let from = string_field(message, "from");
    let recipients = if action.starts_with("review ") {
        unique(vec![
            trimmed_string(task.get("assignedBy")).filter(|id| id != &from),
        ])
    } else {
        let waiting = string_array_from_value(thread.get("waitingOn"))
            .into_iter()
            .filter(|id| id != &from)
            .map(Some)
            .collect::<Vec<_>>();
        let waiting = unique(waiting);
        if !waiting.is_empty() || thread.get("waitingOn").is_some() {
            waiting
        } else {
            unique(vec![
                trimmed_string(task.get("assignedBy")).filter(|id| id != &from),
    ...
    Some(DeliveryPlan {
        prompt: task_outcome_prompt(task, thread, message, action, RECIPIENT_PLACEHOLDER),
        recipients,
    })
}
...
fn deliver_prompt_to_recipients(
    ...
    for recipient in &plan.recipients {
        ...
        if send_prompt_to_tmux(runtime, &window_id, &prompt).is_err() {
            continue;
        }
        if runtime.submit_prompt(&window_id).is_err() {
            continue;
        }
...
fn task_assignment_prompt(
    ...
    let task_kind = if string_field(task, "type") == "review" {
        "review"
    } else {
        "task"
    };
    let mut lines = vec![
        format!("[aimux {task_kind} assigned]"),
        format!("Task: {}", string_field(task, "description")),
        format!("Task id: {}", string_field(task, "id")),
    ];
    if let Some(thread_id) = trimmed_string(thread.get("id")) {
        lines.push(format!("Thread: {thread_id}"));
    }
    lines.extend([
        format!("From: {}", string_field(task, "assignedBy")),
        String::new(),
        format!(
            "Accept before starting: aimux task accept {} --from {recipient}",
            string_field(task, "id")
        ),
        format!(
            "Complete when done: aimux task complete {} --from {recipient} --body \"<summary>\"",
            string_field(task, "id")
        ),
        format!(
            "Block if needed: aimux task block {} --from {recipient} --body \"<reason>\"",
            string_field(task, "id")
        ),
        String::new(),
        "Task prompt:".into(),
        string_field(task, "prompt"),
    ]);
    lines.join("\n")
}

fn task_outcome_prompt(
    task: &Value,
    thread: &Value,
    message: &Value,
    action: &str,
    ...
    let body = trimmed_string(message.get("body"))
        .or_else(|| trimmed_string(task.get("result")))
        .or_else(|| trimmed_string(task.get("error")))
        .unwrap_or_default();
    let mut lines = vec![
        format!("[aimux task {action}]"),
        format!("Task: {}", string_field(task, "description")),
        format!("Task id: {}", string_field(task, "id")),
    ];
    if let Some(thread_id) = trimmed_string(thread.get("id")) {
    ...
    if let Some(from) = trimmed_string(message.get("from")) {
    ...
    lines.extend([
        ...
        body,
        String::new(),
        format!("Inspect: aimux task show {}", string_field(task, "id")),
    ]);
    if let Some(thread_id) = trimmed_string(thread.get("id")) {
        lines.push(format!(
            "Reply: aimux message send \"<reply>\" --thread {thread_id} --from {recipient} --kind reply"
        ));
    }
    lines.join("\n")
}

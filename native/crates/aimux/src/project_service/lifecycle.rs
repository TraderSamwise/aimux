...
fn route_agent_create_teammate(
    context: &ProjectServiceRequestContext,
    body: &Value,
    runtime: &mut impl ProjectLifecycleRuntime,
) -> ProjectServiceDispatchResponse {
...
fn read_displayable_agent_restore_offer(
    context: &ProjectServiceRequestContext,
...
fn reconcile_agent_restore_offer(
    context: &ProjectServiceRequestContext,
    project_state_dir: &Path,
    offer: Option<Value>,
...
fn acknowledge_agent_restore_offer(project_state_dir: &Path) {
...
fn write_agent_restore_retry_offer(project_state_dir: &Path, offer: &Value, failed: &[Value]) {

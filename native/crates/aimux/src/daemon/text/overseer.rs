...
use crate::core_text::{render_core_overseer_clear_lines, render_core_overseer_start_lines};
...
pub trait DaemonOverseerTextRuntime {
...
pub fn route_overseer_text_request(
    runtime: &mut impl DaemonOverseerTextRuntime,
    method: &str,
    path: &str,
    body: Option<&Value>,
    ...
    if method == "POST" && pathname == CORE_API_ROUTES.overseer_start_text {
        return Some(overseer_start_text_route(runtime, &route_url, body));
    }
    if method == "POST" && pathname == CORE_API_ROUTES.overseer_clear_text {
        return Some(overseer_clear_text_route(runtime, &route_url, body));
    }

    None
}

pub fn overseer_start_text_route(
    runtime: &mut impl DaemonOverseerTextRuntime,
    ...
    request.insert("role".into(), Value::String("overseer".into()));
    ...
    let session_id = match required_project_service_string(&json, "overseer start", "sessionId") {
    ...
    text_or_json_lines(
        route_url,
        payload.clone(),
        &render_core_overseer_start_lines(&payload),
    )
}

pub fn overseer_clear_text_route(
    runtime: &mut impl DaemonOverseerTextRuntime,
    route_url: &DaemonRouteUrl,
    body: Option<&Value>,
) -> DaemonRouteResponse {
    ...
    let returned_session_id =
        match required_project_service_string(&json, "overseer clear", "sessionId") {
            Ok(session_id) => session_id,
            Err(response) => return response,
        };
    let payload = json!({
    ...
    text_or_json_lines(
        route_url,
        payload.clone(),
        &render_core_overseer_clear_lines(&payload),
    )
}

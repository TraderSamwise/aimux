...
pub fn parse_sgr_rich_text_contract(text: &str) -> Value {
    let mut attributes = Attributes::default();
    let mut line_texts = Vec::new();
    let lines = text
        .split('\n')
        .map(|line| {
            let spans = parse_line(line, &mut attributes);
            line_texts.push(rich_text_line_text(&spans));
            Value::Array(spans)
        })
        .collect::<Vec<_>>();
    json!({
...
fn parse_line(line: &str, attributes: &mut Attributes) -> Vec<Value> {
    ...
    while let Some(relative) = line[cursor..].find("\u{1b}[") {
        ...
        if start > cursor {
            spans.push(span_of(&line[cursor..start], attributes));
        }
        apply_params(attributes, params);
        cursor = end + 1;
    }
    if cursor < line.len() {
        spans.push(span_of(&line[cursor..], attributes));
    }
    spans
}
...
fn apply_params(attributes: &mut Attributes, params: &str) {
    ...
    let codes = normalized.split(';').collect::<Vec<_>>();
    let mut index = 0;
    while index < codes.len() {
        ...
        match code {
            ...
            Some(code @ 30..=37) => {
                attributes.fg = Some(BASE_16[code as usize - 30].to_owned());
            }
            Some(code @ 40..=47) => {
                attributes.bg = Some(BASE_16[code as usize - 40].to_owned());
            }
            Some(code @ 90..=97) => {
                attributes.fg = Some(BASE_16[code as usize - 90 + 8].to_owned());
            }
            Some(code @ 100..=107) => {
                attributes.bg = Some(BASE_16[code as usize - 100 + 8].to_owned());
            }
            Some(39) => attributes.fg = None,
            Some(49) => attributes.bg = None,
            Some(38 | 48) => {
                let foreground = code == Some(38);
                match codes
                    .get(index + 1)
                    .and_then(|value| value.parse::<u16>().ok())
                {
                    Some(5) => {
                        attributes.set_color(
                            foreground,
                            codes
                                .get(index + 2)
                                .and_then(|value| value.parse::<u16>().ok())
                    ...
                    Some(2) => {
                        let color = match (
                            ...
                            (Some(r), Some(g), Some(b)) => Some(hex(r, g, b)),
                            _ => None,
                        };
                        attributes.set_color(foreground, color);
                        index += 4;
                    }
                    _ => {}
                }
...
impl Attributes {
    fn set_color(&mut self, foreground: bool, color: Option<String>) {
        if foreground {
            self.fg = color;
        } else {
            self.bg = color;
...
fn span_of(text: &str, attributes: &Attributes) -> Value {
...
fn rich_text_line_text(spans: &[Value]) -> String {
    spans
        .iter()

...
pub const CORE_SERVICE_REMOVE_TEXT_ROUTE: &str = "/core/services/remove-text";
...
pub fn native_tool_launch_args_for_config(args: &[String], config: &Value) -> Option<Vec<String>> {
    ...
    if tool == "shell" {
        return shell_service_create_args(extra_args);
    }
...
fn shell_service_create_args(extra_args: &[String]) -> Option<Vec<String>> {
    if extra_args.first().is_some_and(|arg| arg.starts_with('-')) {
        return None;
    }

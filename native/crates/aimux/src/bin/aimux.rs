...
fn core_command_help(args: &[String]) -> Option<&'static str> {
    ...
    match (command, subcommand, help_requested) {
        ...
        ("doctor", Some("coherence"), true) => Some(DOCTOR_COHERENCE_HELP),
        ...
        ("job", Some("tail"), true) => Some(JOB_TAIL_HELP),
        ("job", Some("wait"), true) => Some(JOB_WAIT_HELP),
        ("job", Some("cancel"), true) => Some(JOB_CANCEL_HELP),
        ("job", Some("notify"), true) => Some(JOB_NOTIFY_HELP),
        ...
        ("worktree", Some("open"), true) => Some(WORKTREE_OPEN_HELP),
        ("worktree", Some("prune"), true) => Some(WORKTREE_PRUNE_HELP),
        ...
        ("graveyard", Some("reap-dead"), true) => Some(GRAVEYARD_REAP_DEAD_HELP),
...
const DOCTOR_COHERENCE_HELP: &str = "Usage: aimux doctor coherence [options]\n\nCheck the durable record against the live runtime\n\nOptions:\n  --project-root <path>       Project root\n  --repair                    Apply the repairs that are safe to apply\n  --json                      Emit JSON";
...
const INPUT_HELP: &str = "Usage: aimux input <sessionId> <text...> [options]\n\nSubmit input to a running agent as its next user turn. This is not side-channel context and can interrupt in-flight work.\n\nOptions:\n  --project <path>            Project path\n  --force                     Send even when the target is not waiting";
...
const WORKTREE_CREATE_HELP: &str = "Usage: aimux worktree create <name> [options]\n\nCreate a linked git worktree under .aimux/worktrees; use this for agent work instead of git clone\n\nOptions:\n  --project <path>            Project path\n  --pr <number>               Check out an open GitHub pull request into the new worktree\n  --branch <branch>           Check out origin/<branch> into the new worktree\n  --json                      Emit JSON";
const WORKTREE_OPEN_HELP: &str = "Usage: aimux worktree open <github-url> [options]\n\nOpen a GitHub pull request or branch URL in a tracking Aimux worktree\n\nOptions:\n  --project <path>            Project path\n  --json                      Emit JSON";
const WORKTREE_PRUNE_HELP: &str = "Usage: aimux worktree prune [options]\n\nPrune stale git worktree metadata with git worktree prune. Defaults to dry-run and never force-removes checkout directories.\n\nOptions:\n  --project <path>            Project path\n  --yes                       Prune instead of dry run\n  --json                      Emit JSON";
...
const GRAVEYARD_REAP_DEAD_HELP: &str = "Usage: aimux graveyard reap-dead [id] [options]\n\nMove confirmed-dead inventoried agents to the graveyard. A session is reaped only after its tmux session is queried successfully and its expected window is absent.\n\nOptions:\n  --project <path>            Project path\n  --json                      Emit JSON";
...
const RUN_HELP: &str = "Usage: aimux run <address> [args...] --tool <toolKey> (--skill <name> | --prompt <text>) [options]\n\nRun or join the one live transient job at an address. Ctrl-C detaches from the stream and leaves the job running; use `aimux job cancel <handle>` to cancel. A different live spec at the same address exits 24.\n\nOptions:\n  --tool <toolKey>            Configured tool key that executes the job\n  --skill <name>              Invoke the tool with /<name> plus args\n  --prompt <text>             Invoke the tool with raw prompt text plus args\n  --notify-fifo <path>        Write one JSON line to a caller-owned FIFO on terminal status\n  --project <path>            Project path used to resolve address handles\n  --detach                    Create or join the job, print its id, and exit\n  --json                      Emit NDJSON job events while streaming";
const JOB_HELP: &str = "Usage: aimux job [command]\n\nInspect, tail, wait for, and cancel transient jobs\n\nCommands:\n  show <handle>               Show one job by id or address\n  list                        List jobs; --scope is recursive by default\n  tail <handle>               Tail job events from the daemon stream\n  wait <handle>               Wait for terminal status, print nothing, and exit with the job code\n  cancel <handle>             Request cancellation\n  notify <handle>             Notify this desktop when the job reaches terminal status";
const JOB_SHOW_HELP: &str = "Usage: aimux job show <handle> [options]\n\nShow one job by id or scope address\n\nOptions:\n  --project <path>            Project path used to resolve address handles\n  --json                      Emit JSON";
const JOB_LIST_HELP: &str = "Usage: aimux job list [options]\n\nList transient jobs\n\nOptions:\n  --scope <address>           Filter by address prefix recursively by default\n  --depth <n>                 Include the address plus up to n additional segments; 0 is exact only\n  --project <path>            Project path used to resolve scope names\n  --json                      Emit JSON";
const JOB_TAIL_HELP: &str = "Usage: aimux job tail <handle> [options]\n\nTail job events. Ctrl-C detaches from the stream and leaves the job running.\n\nOptions:\n  --project <path>            Project path used to resolve address handles\n  --seq <seq>                 First event sequence to stream\n  --json                      Emit NDJSON";
const JOB_WAIT_HELP: &str = "Usage: aimux job wait <handle> [options]\n\nWait until terminal status, print nothing, and exit with the job's status code.\n\nOptions:\n  --project <path>            Project path used to resolve address handles\n  --seq <seq>                 First event sequence to stream";
const JOB_CANCEL_HELP: &str = "Usage: aimux job cancel <handle> [options]\n\nCancel a transient job\n\nOptions:\n  --project <path>            Project path used to resolve address handles\n  --json                      Emit JSON";
const JOB_NOTIFY_HELP: &str = "Usage: aimux job notify <handle> [options]\n\nRegister a local desktop notification for job completion\n\nOptions:\n  --project <path>            Project path used to resolve address handles\n  --watcher-id <id>           Stable watcher id for this notification registration\n  --json                      Emit JSON";

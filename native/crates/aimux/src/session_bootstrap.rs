...
pub fn seed_fork_artifacts(
    ...
    if let Some(live) = snapshot.live_text.as_deref() {
        let _ = write_text_atomic(target_context.join("live.md"), format!("{live}\n"));
    }
    let summary = [
...
pub fn build_fork_preamble(
    ...
    [
        ...
        snapshot
            .live_text
            .as_ref()
            .map(|_| format!("- {}", target_context_dir.join("live.md").to_string_lossy()))
            .unwrap_or_default(),
        format!(
            "- {}",

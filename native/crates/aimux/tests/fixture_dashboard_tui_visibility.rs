...
#[test]
fn fixture_dashboard_tui_visibility_matches_typescript() {
    let contract: Value = serde_json::from_str(DASHBOARD_TUI_VISIBILITY)
        .expect("dashboard TUI visibility fixture parses");
    let cases = contract["cases"].as_array().expect("visibility cases");
    assert_eq!(cases.len(), 18, "unexpected visibility case count");
...
fn run_case(case: &Value) -> Value {
    ...
    match case["api"].as_str().expect("case api") {
        ...
        "readTmuxTuiVisibility" => serde_json::to_value(read_tmux_tui_visibility_from_values(
            input["env"]["TMUX_PANE"].as_str(),
        ...
        .expect("snapshot json"),
        "parseTmuxPaneRows" => {
            serde_json::to_value(parse_tmux_pane_rows(input["raw"].as_str())).expect("pane rows")
        }
        ...
        "readDashboardTuiVisibilityForHost" => run_host_case(case),
        "markDashboardTuiVisible" => run_mark_visible_case(),
        other => panic!("unexpected visibility api {other}"),
    }
}

fn run_host_case(case: &Value) -> Value {
    match case["name"].as_str().expect("case name") {
        "does not query tmux for non-dashboard hosts" => {
            ...
            json!({
                "snapshot": read_dashboard_tui_visibility_for_state(&mut state, false, 0, || {
        ...
        "caches visibility briefly" => {
            ...
            json!({
                ...
                "second": second,
                "calls": calls,
                "host": host_json(&state, HostJsonOptions { started: true, wake: false, hidden_zeroes: false }),
            })
        }
        "records a wake transition when a hidden dashboard becomes visible" => {
            ...
            json!({
                ...
                "host": host_json(&state, HostJsonOptions { started: true, wake: true, hidden_zeroes: false }),
            })
        }
...
fn run_mark_visible_case() -> Value {
    ...
    let second_wake = consume_dashboard_tui_visibility_wake(&mut state);
    json!({
        "host": host_json(&state, HostJsonOptions { started: false, wake: true, hidden_zeroes: true }),
        "firstWake": first_wake,
        "secondWake": second_wake,
    })
}

struct HostJsonOptions {
    started: bool,
    wake: bool,
    hidden_zeroes: bool,
}

fn host_json(state: &DashboardTuiVisibilityState, opts: HostJsonOptions) -> Value {

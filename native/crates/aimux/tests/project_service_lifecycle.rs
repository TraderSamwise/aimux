...
fn agent_stop_reports_tmux_kill_failure_without_taking_session_offline() {
    ...
    let desktop_failures = desktop.body["operationFailures"]
        .as_array()
        .expect("desktop operation failures");
    assert_eq!(desktop_failures.len(), 1);
    assert_eq!(desktop_failures[0]["operation"], "agent.stop");
    assert_eq!(desktop_failures[0]["targetId"], "codex-live");
    let desktop_session = desktop.body["sessions"]
        .as_array()
...
fn agent_kill_reports_tmux_kill_failure_instead_of_graveyard_success() {
    ...
    let desktop_failures = desktop.body["operationFailures"]
        ...
        .expect("desktop operation failures");
    assert_eq!(desktop_failures.len(), 1);
    assert_eq!(desktop_failures[0]["operation"], "agent.kill");
    assert_eq!(desktop_failures[0]["targetId"], "codex-live");
    let desktop_session = desktop.body["sessions"]
        .as_array()
        .expect("desktop sessions")

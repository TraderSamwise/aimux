...
fn local_context(headers: &BTreeMap<String, String>) -> DaemonRouteRequestContext {
...
fn stopping_short_circuits_before_cors_like_daemon_start_handler() {
    ...
    let response = handle_daemon_http_request(
        req,
        |_, _, _, headers| local_context(headers),
        |_, _, _, _, _| unreachable!("route skipped"),
    );

    assert_eq!(response.status, 503);
    assert_eq!(
        response.body,
        br#"{"ok":false,"error":"aimux daemon is stopping"}"#
    );
...
fn cors_and_options_are_handled_before_route_dispatch() {
    ...
    assert_eq!(
        response.body,
        br#"{"ok":false,"error":"origin not allowed"}"#
    );
...
fn post_body_is_parsed_like_daemon_handle_except_restart_text() {
    ...
    let response = handle_daemon_http_request(
        req,
        |_, _, _, headers| local_context(headers),
        |method, path, body, _context, issued_at| {
...
fn form_body_parse_and_json_parse_errors_match_handle_contract() {
    ...
    form.body_chunks
        .push(b"title=Hello+Sam&encoded=a%2Fb".to_vec());
    let response = handle_daemon_http_request(
        form,
        |_, _, _, headers| local_context(headers),
        |_, _, body, _, _| DaemonRouteResponse::json(200, body.cloned().unwrap()),
    );
...
fn route_context_carries_headers_actor_and_access_decision() {
    ...
    handle_daemon_http_request(
        req,
        |_, _, _, headers| DaemonRouteRequestContext {
            actor_present: true,
            headers: headers.clone(),
            access_decision: Some(RemoteAccessDecision::deny(403, "nope")),
        },
        |_, _, _, context: &DaemonRouteRequestContext, _| {
...
fn prepared_response_preserves_text_and_binary_route_bodies() {
    let text = handle_daemon_http_request(
        request("GET", "/text"),
        |_, _, _, headers| local_context(headers),
        |_, _, _, _, _| DaemonRouteResponse::text(200, "hello\n"),
    );
    ...
    let binary = handle_daemon_http_request(
        request("GET", "/image"),
        |_, _, _, headers| local_context(headers),
        |_, _, _, _, _| DaemonRouteResponse {
            status: 200,

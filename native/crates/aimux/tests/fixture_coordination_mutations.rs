...
const COORDINATION_MUTATIONS: &str =
    include_str!("../../../../testdata/contracts/v1/coordination/mutations.json");
...
impl TestProject {
    fn new(label: &str) -> Self {
        let root = std::env::temp_dir().join(format!(
            "aimux-coordination-mutations-fixture-{label}-{}-{}",
            std::process::id(),
...
#[test]
fn fixture_orchestration_mutations_match_typescript() {
    let contract: Value =
...
fn run_case(project: &TestProject, input: &Value) -> Value {
    ...
    for (index, operation) in input["operations"]
        .as_array()
        .expect("operations")
        .iter()
        .enumerate()
        ...
        match input["scenario"].as_str().unwrap_or_default() {
            "direct-reuse" => {
                results.insert(
                    ...
                    strip_route_only_fields(response.body),
                );
            }
            "thread-reply" => {
                results.insert(
                    if index == 0 { "created" } else { "replied" }.into(),
                    strip_route_only_fields(response.body),
                );
            }
            _ => {}
...
impl DynamicNormalizer {
    ...
    fn normalize_string(&mut self, input: &str) -> String {
        let with_timestamps = replace_timestamps(input);
...
fn replace_timestamps(input: &str) -> String {
    ...
    while index < input.len() {
        if let Some(candidate) = input.get(index..index + 24)
            && is_iso_timestamp(candidate)
        {
            output.push_str("<ts>");
            index += 24;
        } else {
...
fn replace_dynamic_ids(input: &str, mut token: impl FnMut(&str) -> String) -> String {
    ...
    let mut index = 0;
    while index < input.len() {
        if let Some(end) = dynamic_id_end(&input[index..]) {
...
fn dynamic_id_end(input: &str) -> Option<usize> {
    for prefix in [
        "task-",
        "thread-",
        "msg-",

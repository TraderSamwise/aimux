...
| Claude/Codex hooks | `shell/hooks.json`; no-Node hook POST smoke | Yes for captured hook payload parsing/settings emission and project-service hook routes | Native | Project-service hook routes survive. Live external-tool invocation ordering is still a residual integration risk. |
...
| Agent coordination/orchestration | Implemented for captured semantics | `coordination/*.json`, `orchestration/*.json`, and `coordination/scribe-watcher.json` are `PROVEN-FAILS` | Cross-process delivery timing remains residual risk. |
| Hooks and default plugins | Implemented for captured contracts; public plugin API suspended | `shell/hooks.json`, `default-plugins/gh-pr-context.json`, `default-plugins/transcript-length.json`, `plugin/runtime.json`, `fixture_plugin_api`, and `project_service_reads` are proven; hook route smoke | The native built-ins run through the internal serializable plugin API and are wired into project-service diagnostics. Runtime execution of arbitrary user JS plugins is intentionally unavailable for phase 8. |
...
| Static/source boundary helpers | Implemented as guards | `source-boundary/inventory.json`, `paths/*.json`, `config/*.json`, `install-config/config.json` are `PROVEN-FAILS` | These prove invariants and shape, not live runtime behavior. |

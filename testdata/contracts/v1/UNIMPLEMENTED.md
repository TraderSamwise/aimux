...
| `src/dashboard/command-spec.test.ts` | Dashboard command spec behavior exists but needs TS output fixture for the legacy launcher contract. | dashboard_* |
...
| `src/tmux/sync-exec-inventory.test.ts` | Source-inventory guard for sync tmux calls; needs real violation-list capture, not reporter status. | tmux* |

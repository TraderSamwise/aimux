...
record(
  ...
  "deriveAimuxSessionIdFromBackendSessionId+summarizeLaunchArgs+injectCodexDeveloperInstructions",
  {
    ...
    codexArgsDash: ["--model", "gpt-5", "--", "literal prompt"],
    codexArgsShort: ["-m", "gpt-5", "-s", "workspace-write", "Implement the parser"],
    codexArgsNoPositional: ["--model=gpt-5", "--config", "foo=bar"],
  ...
  (input) => ({
    ...
    summarizedEdge: summarizeLaunchArgs(input.launchArgsEdge),
    injected: injectCodexDeveloperInstructions(input.codexArgs, input.developerKey, input.instructions),
    injectedBeforeDash: injectCodexDeveloperInstructions(input.codexArgsDash, input.developerKey, input.instructions),
    injectedAfterShortOptions: injectCodexDeveloperInstructions(input.codexArgsShort, input.developerKey, input.instructions),
    injectedWithoutPositional: injectCodexDeveloperInstructions(
      input.codexArgsNoPositional,
      input.developerKey,
      input.instructions,
    ),
    blankInjection: injectCodexDeveloperInstructions(input.codexArgs, "", input.instructions),
    blankInstructionsInjection: injectCodexDeveloperInstructions(input.codexArgs, input.developerKey, "  "),
  }),
);

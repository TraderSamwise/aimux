...
show_local_expose() {
  ...
  if [ -n "$popup_client_tty" ]; then
    tmux display-popup -c "$popup_client_tty" -T "aimux exposé" -x C -y C -w 100% -h 100% -B -E "$expose_cmd" >/dev/null 2>&1 || return 1
  else
    tmux display-popup -T "aimux exposé" -x C -y C -w 100% -h 100% -B -E "$expose_cmd" >/dev/null 2>&1 || return 1
  fi
  exit 0
}

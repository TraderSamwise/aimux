...
export async function listGlobalExposeItems(
...
): Promise<{ ok: boolean; items: unknown[]; projectReadErrors?: DaemonProjectReadError[] }> {
  ...
  if (shouldRouteViaRelay())
    return callDaemonViaRelay<{
      ok: boolean;
      items: unknown[];
      projectReadErrors?: DaemonProjectReadError[];
    }>("GET", path, undefined, apiOpts);
  return callJson<{ ok: boolean; items: unknown[]; projectReadErrors?: DaemonProjectReadError[] }>(
    `${getDaemonUrl()}${path}`,
    { method: "GET" },
    apiOpts,
  );

...
export class Multiplexer {
  ...
  createSession(
    ...
    // Append user preamble from AIMUX.md if it exists
    const aimuxMdPath = join(process.cwd(), "AIMUX.md");
    if (existsSync(aimuxMdPath)) {
      try {
        const userPreamble = readFileSync(aimuxMdPath, "utf-8").trim();
        if (userPreamble) {
          preamble += "\n\n" + userPreamble;
          debug(`loaded AIMUX.md (${userPreamble.length} chars)`, "preamble");
        }
      } catch {}
    }
  ...
  private writeInstructionFiles(): void {
    ...
    // Append user preamble from AIMUX.md
    let fullPreamble = preamble;
    const aimuxMdPath = join(process.cwd(), "AIMUX.md");
    if (existsSync(aimuxMdPath)) {
      try {
        const userContent = readFileSync(aimuxMdPath, "utf-8").trim();
        if (userContent) {
          fullPreamble += "\n## User Instructions\n\n" + userContent + "\n";
        }
      } catch {}

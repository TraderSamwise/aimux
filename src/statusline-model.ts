...
export function resolveScopedSessions(
  ...
  const scopedSessions = (data.sessions ?? [])
    ...
    .filter((session) => session.overseer !== true)
...
export function resolveFocusedOverseer(
  data: StatuslineData,
  projectRoot: string,
  currentSession?: string,
  currentWindow?: string,
  ...
  const overseer = (data.sessions ?? []).find((session) => session.id === exactCurrentSessionId && session.overseer);
  if (!overseer || overseer.status === "offline" || overseer.status === "exited") return null;
  const resolvedMetadata = data.metadata?.[overseer.id];
  return {
    ...overseer,
    derived: resolvedMetadata?.derived,
    semantic: overseer.semantic,
    metadata: resolvedMetadata,
    isCurrent: true,
  };
}

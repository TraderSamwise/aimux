...
export class Dashboard {
  ...
  private buildHelpLine(): string {
    ...
    if (this.sessions.length === 0 && !this.hasWorktrees) {
      return " [u] attention  [a] activity  [t] threads  [Tab] details  [c] new  [f] fork  [p] plans  [g] graveyard  [?] help  [q] quit ";
    }
    ...
    if (this.sessions.length > 0) {
      const xPart = xLabel ? `  ${xLabel}` : "";
      return ` ↑↓ select  ${enterLabel}  [u] attention  [a] activity  [t] threads  [Tab] details  [c] new  [f] fork  [w] worktree${xPart}${rLabel}${tmuxHint}  [p] plans  [g] graveyard  [?] help  [q] quit `;
    }
    return " [u] attention  [a] activity  [t] threads  [Tab] details  [c] new  [f] fork  [w] worktree  [p] plans  [g] graveyard  [?] help  [q] quit ";
  }

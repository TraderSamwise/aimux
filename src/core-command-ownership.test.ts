...
const coreCommandDispositions: Array<{
  ...
  { command: "daemon ensure --json", args: ["daemon", "ensure", "--json"], disposition: "node-core-fallback" },
  {
  ...
  { command: "host status", args: ["host", "status"], disposition: "node-core-fallback" },
  { command: "host status --json", args: ["host", "status", "--json"], disposition: "node-core-fallback" },
  {
...
describe("core command ownership inventory", () => {
  ...
  it("keeps shim-fast-path commands backed by explicit installed shell routes", () => {
    ...
    expect(fastPaths).toHaveLength(7);

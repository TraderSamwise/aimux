...
describe("installed aimux shim", () => {
  ...
  it("serves doctor and repair commands from a matching daemon without launching Node", () => {
    ...
    expect(fixture.run(["doctor", "exchange", "--project", "/repo", "--json"]).stdout).toBe("doctor ok\n");
    expect(fixture.run(["doctor", "lifecycle", "--project=/repo"]).stdout).toBe("doctor ok\n");
    expect(
    ...
    expect(curlLog).toContain("/core/doctor/exchange-text?json=1");
    expect(curlLog).toContain("/core/doctor/lifecycle-text");
  ...
  it("defaults host agent-stream to the bounded scrollback window", () => {
    const fixture = makeFixture();
    writeFileSync(fixture.healthFile, `${health("build-1", 321)}\n`);
    writeFileSync(fixture.textRouteFile, "streamed output\n");
    writeFileSync(fixture.daemonInfoPath, `${JSON.stringify({ pid: 321, port: 45678 })}\n`);
    ...
    expect(result.status).toBe(0);
    const curlLog = readFileSync(fixture.curlLog, "utf8");
    expect(curlLog).toContain("startLine=-2000\n");
    expect(curlLog).toContain("intervalMs=500\n");
    expect(existsSync(fixture.nodeLog)).toBe(false);
  });
  ...
  it("serves worktree commands from a matching daemon without launching Node", () => {
    ...
    expect(
      fixture.run(
        ["worktree", "cleanup-caches", "--project", "/repo", "--include-active", "--yes"],
        {},
        {
          cwd: projectDir,
        },
    ...
    expect(curlLog).toContain("dryRun=0\n");
    expect(curlLog).toContain("includeActive=1\n");
  ...
  it("falls through to Commander for workflow help and missing required positional arguments", () => {
    ...
    expect(fixture.run(["worktree", "cleanup-caches", "--help"], { NODE_EXIT: "0" }).status).toBe(0);
    expect(fixture.run(["graveyard", "cleanup", "--help"], { NODE_EXIT: "0" }).status).toBe(0);
    ...
    expect(readFileSync(fixture.nodeLog, "utf8")).toBe(
      [
        ...
        `${fixture.aimuxRoot}/dist/launcher-bin.js worktree cleanup-caches --help`,
        `${fixture.aimuxRoot}/dist/launcher-bin.js graveyard cleanup --help`,

 ...
 *   after every embedded reset so a pre-styled frame stays uniformly dimmed; a harmless
 *   `faint+reset` may trail. Faint is widely but not universally honored — it degrades
 *   to a no-op on terminals that ignore it.
...
export function recede(text: string, mode: RecedeMode = "faint"): string {
  ...
  if (mode === "faint") {
    const faint = "\x1b[2m";
    return `${faint}${text.replace(/\x1b\[0?m/g, `${RESET}${faint}`)}${RESET}`;
  }

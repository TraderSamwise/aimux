...
// Gray floor for faint backdrops so uncolored text dims further.
const FAINT_FG = 240;

/**
 * Recede content visually so a foreground layer (a modal) reads above it.
 *
 * Keeps the content's own colors but dims them, over a gray foreground floor so
 * uncolored text recedes further. The lead is re-injected after every embedded reset
 ...
 */
export function recede(text: string): string {
  if (text === "") return "";
